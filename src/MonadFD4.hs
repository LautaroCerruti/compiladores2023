...
printInlineFD4 :: MonadFD4 m => String -> m ()
printInlineFD4 = liftIO . putStr
...
lookupNameTy :: MonadFD4 m => Name -> m (Maybe Ty)
lookupNameTy nm = do
     s <- get
     case filter (\(name,_) -> nm == name) (glbTy s) of
       (_,ty):_ -> return (Just ty)
       [] -> return Nothing
